"""Drive-based Monte Carlo simulator for NFL games (SHADOW model, `drive_sim_v1`).
...
def prep_drives(d: pd.DataFrame, games: pd.DataFrame) -> pd.DataFrame:
    """Drive rows -> model frame. `games` supplies game_id -> location (Neutral/Home)."""
...
def _features(off, de, hadv, yl, tb, per, sdb, variant, T):
    ...
    if variant == "c":
        ...
        S = np.delete(S, [p * NSD + 4 for p in range(NPER)], axis=1)  # tied = base per period
...
class DriveModel:
    def __init__(self, variant=VARIANT, C=C, tau=TAU_WEEKS, ktau=KTAU_WEEKS,
                 pace_alpha=PACE_ALPHA):
    ...
    def fit(self, tr: pd.DataFrame, t_now: int, teams: pd.Index):
        ...
        for o in range(NO):
            for b in range(NTB):
                ...
                qb = wquant(dur[base], w_t[base]) if base.sum() > 0 else np.full(Q, 60.0)
                for g in range(3):
                    m = base & (sg == g)
                    D[o, b, g] = wquant(dur[m], w_t[m]) if m.sum() > 30 else qb
        self.D = D
...
def fit_week(drives: pd.DataFrame, games: pd.DataFrame, season: int, week: int,
             teams: pd.Index | None = None) -> DriveModel:
    """Fit on drives in the FIT_SEASONS before (season, week), strictly earlier weeks."""
    d = drives if "t" in drives.columns else prep_drives(drives, games)
    t_now = time_index(season, week)
    tr = d[(d.t < t_now) & (d.t >= t_now - FIT_SEASONS * WPS)]
    if len(tr) < 2000:
    ...
    return DriveModel().fit(tr, t_now, teams)
...
def summarize_game(hs: np.ndarray, aw: np.ndarray, went_ot: np.ndarray,
                   spread_line: float | None = None, total_line: float | None = None) -> dict:
    ...
    out["p_over_by_line"] = json.dumps({_line_key(L): round(float((tot > L).mean()), 5)
                                        for L in lines})
    H = np.bincount(np.clip(mg, -80, 80).astype(int) + 80, minlength=len(MARGIN_GRID)).astype(float)
    P = H / H.sum()
    if has_spread:
        P = tilt(0.995 * P + 0.005 / len(P), MARGIN_GRID, float(spread_line))
        basis = "sim_shape_at_spread"
    else:
        basis = "raw_sim"
    out["margin_exact"] = json.dumps({str(k): round(float(P[k + 80]), 5) for k in MARGIN_EXACT_K})
...
def simulate_slate(slate: pd.DataFrame, drives: pd.DataFrame, games: pd.DataFrame,
                   season: int, week: int, n: int = N_SIMS, seed: int = SEED,
                   now: datetime | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    t0 = time.time()
    model = fit_week(drives, games, season, week)
    fit_s = time.time() - t0
    dist_rows, pred_rows = [], []
    t1 = time.time()
    for r in slate.itertuples(index=False):
        home, away = RELOCATED.get(r.home_team, r.home_team), RELOCATED.get(r.away_team, r.away_team)
        if home not in model.teams or away not in model.teams:
            ...
            continue
        hs, aw, ot = simulate(model, home, away, getattr(r, "location", "Home") == "Neutral",
        ...
        pred_rows.append(dict(
            **{**ident, "game_date": pd.Timestamp(gd).tz_localize("UTC")}, game_pk=abs(zlib.crc32(str(r.game_id).encode())),
            ...
            model_vs_vegas_edge=None if vegas is None else p - vegas))
    timing = {"fit_s": round(fit_s, 2), "sim_ms_per_game":
              ...
              "drives_in_window": int(((drives.t if "t" in drives else drives.season * WPS + drives.week)
                                       >= time_index(season, week) - FIT_SEASONS * WPS).sum())}
